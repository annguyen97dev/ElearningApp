...
const Quiz = ({ dataLesson }) => {
			...
			setTimeout(() => {
				$(document).ready(function () {
					...
					if (saveParagraph.length > 0) {
						saveParagraph.forEach((item) => {
							if (item.indexDetail === activeQuestion.indexDetail) {
								document.getElementById(item.areaID).outerHTML = item.textArea;
							}
						});
					}
						...
						.mouseup(function (event) {
							...
							if (nodeName !== 'INPUT') {
								...
								if (sel && sel.rangeCount > 0) {
									...
									console.log('Nodes Arr when mouse up: ', nodesArr);
									...
									console.log('RANGE when mouse up: ', range);
									console.log('Area ID when mouseup: ', areaID);
									// ---------- -----------------------
					...
					function checkAllCondition(dataId, range, nodesArr) {
						...
						console.log('NodesArr in check: ', nodesArr);
	...
	}, [open, activeQuestion.indexDetail]);
					...
					for (const [index, item] of itemAns.entries()) {
						...
						if (
							...
							if (index == itemAns.length - 1) {
								...
								let classItem = item.nextElementSibling.childNodes[0].className;
					...
					for (const [index, item] of itemAns.entries()) {
						...
						if (
							...
							if (index == 0) {
								...
								if (classItem === 'checked') {
									itemAns[itemAns.length - 1].classList.add('activeDoingFirst');
								} else {
